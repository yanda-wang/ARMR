...
from Networks import EncoderRNNQuery, DecoderMultiHopKeyValueSeparate
from Networks import DiscriminatorMLPPremium
...
class TrainMedRec:
    ...
    def loss_function(self, target_medications, predict_medications, proportion_bce, proportion_multi):
        """
        loss based on binary cross entropy (bce) loss and multi-label loss (multi)
        :param target_medications: target medications
        :param predict_medications: predict medications
        :param proportion_bce: proportion for the bce loss
        :param proportion_multi: proportion for the multi loss
        """
    ...
    def get_performance_on_testset(self, encoder, decoder, data_loader, proportion_bce=0.9, proportion_multi=0.1):
        """
        get performance on the test dataset
        """
        data_loader.shuffle()
    ...
    def trainIters(self, encoder, decoder, encoder_optimizer, decoder_optimizer, data_loader_train, data_loader_test,
        ...
        """
        train the MedRec model
        :param encoder:
        :param decoder:
        :param encoder_optimizer:
        :param decoder_optimizer:
        :param data_loader_train: data loader for model training
        :param data_loader_test: data loader for evaluating the model on the test dataset
        :param save_model_path: save the model to the directory
        :param n_epoch: #epoch for training
        :param print_every_iteration: print the performance every print_every_iteration iterations
        :param save_every_epoch: save the model every save_every_epoch epochs
        :param proportion_bce: proportion for bce loss
        :param proportion_multi: proportin for multi-label loss
        :param medrec_trained_epoch: #epoch the model has been trained
        :param medrec_trained_iteration: #iterations the model has been trained
        :param gan_trained_epoch: if the model has been regularized by the GAN model, #epoch the model has been regularized
        :param gan_trained_iteration: if the model has been regularized by the GAN model, #iterations the model has been regularized
        :param gan_patient_ddi_rate: if the model has been regularized by the GAN model, real data are generated based on patients with ddi rate lower than gan_patient_ddi_rate
        :return:
        """
        ...
        for epoch in range(start_epoch, start_epoch + n_epoch):
            ...
            jaccard_avg, precision_avg, recall_avg, f1_avg, delta_ddi_rate, coverage, print_loss_avg_on_test, prauc_avg = self.get_performance_on_testset(
                encoder, decoder, data_loader_test, proportion_bce, proportion_multi)
            encoder.train()
            decoder.train()
    ...
    def train(self, input_size, hidden_size, encoder_n_layers, encoder_embedding_dropout_rate, encoder_gru_dropout_rate,
        ...
        """
        train MedRec
        :param input_size: input size of encoder
        :param hidden_size: hidden size of the RNNs in the encoder
        :param encoder_n_layers: #layers in the RNNs of the encoder
        :param encoder_embedding_dropout_rate: dropout rate for the embedding matrix in the encoder
        :param encoder_gru_dropout_rate: dropout rate for the RNNs in the encoder
        :param encoder_bidirectional: whether the RNNs in the encoder are bidirectional or not
        :param encoder_regular_lambda: weight decay of the encoder optimizer
        :param encoder_learning_rate: learning rate of the encoder optimizer
        :param decoder_dropout_rate: dropout rate for the decoder
        :param decoder_regular_lambda: weight decay of the decoder optimizer
        :param decoder_learning_rate: learning rate of the decoder optimizer
        :param hop: #hops carried out in the decoder
        :param attn_type_kv: attention function for patient representations and keys of the key-value memory networks, choose from dot,concat,and general
        :param attn_type_embedding: attention function for patient representations and the embedding matrix in decoder, hoose from dot,concat,and general
        :param save_model_dir: save model to the directory
        :param proportion_bce: proportion for the bce loss
        :param proportion_multi: proportion for the multi-label loss
        :param medrec_patient_ddi_rate: patient records whose ddi rate is lower than medrec_patient_ddi_rate are used for traning
        :param n_epoch: #epochs for training
        :param print_every_iteration: print the performance every print_every_iteration iterations
        :param save_every_epoch: save the model every save_every_epoch epochs
        :param load_model_name: load model from this file if it is not None
        :param pretrained_embedding_diagnoses:
        :param pretrained_embedding_procedures:
        :param pretrained_embedding_medications:
        :param discriminator_structure: string, if the model has been regularized by a GAN model, this string indicates the structure of the discriminator
        :param discriminator_parameters: string, if the model has been regularized by a GAN model, this string indicates the parameters of the discriminator
        :return:
        """
        ...
        encoder = EncoderRNNQuery(self.device, input_size, hidden_size, self.diagnoses_count, self.procedures_count,
                                  encoder_n_layers, encoder_embedding_dropout_rate, encoder_gru_dropout_rate,
                                  embedding_diagnoses_np, embedding_procedures_np,
                                  bidirectional=encoder_bidirectional)
        decoder = DecoderMultiHopKeyValueSeparate(self.device, hidden_size, self.medication_count,
                                                  self.medication_count, hop, embedding_medications_np,
                                                  decoder_dropout_rate, attn_type_kv, attn_type_embedding)
        ...
        if discriminator_structure is not None:
            save_model_structure = str(encoder_n_layers) + '_' + str(input_size) + '_' + str(hidden_size) + '_' + str(
                encoder_bidirectional) + '_' + str(attn_type_kv) + '_' + str(attn_type_embedding) + '_' + str(
            ...
            save_model_structure = str(encoder_n_layers) + '_' + str(input_size) + '_' + str(hidden_size) + '_' + str(
                encoder_bidirectional) + '_' + str(attn_type_kv) + '_' + str(attn_type_embedding) + '_None'
            save_model_parameters = str(encoder_embedding_dropout_rate) + '_' + str(
                encoder_gru_dropout_rate) + '_' + str(encoder_regular_lambda) + '_' + str(
                encoder_learning_rate) + '_' + str(decoder_dropout_rate) + '_' + str(
    ...
    def get_hidden_states_between_thresholds(self, input_size, hidden_size, encoder_n_layers, encoder_bidirectional,
        ...
        :param load_model_name:
        :param patient_recodrs_file:
        :param data_mode: choose from train, test, and validation
        :param medrec_patient_ddi_rate_low:
        :param medrec_patient_ddi_rate_high:
        :param file_save_path: save the generated hidden states to the file
        :param encoder_structure_str: string, indicates the structure of the encoder
        :param encoder_parameters_str: string, indicates the parameters of the encoder
        :return:
        """

        print('initializing >>>')
        encoder = EncoderRNNQuery(self.device, input_size, hidden_size, self.diagnoses_count, self.procedures_count,
                                  encoder_n_layers, bidirectional=encoder_bidirectional)
    ...
    def isPD(self, B):
        """
        check whether B is positive definite or not
        :param B:
        :return:
        """
        try:
    ...
    def nearest_PD(self, A):
        """
        find the most approximate positive definite matrix for A
        :param A:
        :return:
        """
        B = (A + A.T) / 2
    ...
    def fit_gaussian_from_encoder(self, input_size, hidden_size, encoder_n_layers, encoder_bidirectional,
                                  ...
                                  standard=False):
        """
        fit a gaussian distribution according to the encoder outputs of patients with ddi rate smaller than medrec_patient_ddi_rate
        :param input_size:
        :param hidden_size:
        :param encoder_n_layers:
        :param encoder_bidirectional:
        :param load_model_name:
        :param medrec_patient_ddi_rate:
        :param real_data_save_path: save the data sampled from the fitted distribution to this directory
        :param encoder_structure_str:
        :param encoder_parameters_str:
        :param decimals:
        :param boxcox_transform: whether to carry out boxcox transform on the encoder outputs before fit a distribution
        :param boxcox_lambda: transform type for boxcox transform
        :param standard: whether use identical matrix as the cov matrix or not
        :return:
        """
...
class TrainMedRecGANPremiumSingleDisNormal:
    ...
    def loss_function(self, target_medications, predict_medications, proportion_bce, proportion_multi):
        ...
        :param target_medications: target of medications
    ...
    def train_iters(self, encoder, decoder, discriminator, encoder_optimizer, decoder_optimizer, generator_optimizer,
                    discriminator_optimizer, data_loader_medrecGAN, data_loader_medrec_test, save_model_path, n_epoch,
                    print_every_iteration=100, save_every_epoch=5, gan_single_iteration=5, proportion_bce=0.9,
                    proportion_multi=0.1, trained_epoch=0, trained_medrec_iterations=0, trained_gan_iterations=0,
                    gan_patient_ddi_rate=0):
        """
        :param encoder:
        :param decoder:
        ...
        :param encoder_optimizer:
        :param decoder_optimizer:
        :param generator_optimizer: optimizer for the generator, which optimizes the encoder
        :param discriminator_optimizer: optimizer for the discriminator
        :param data_loader_medrecGAN: data loader for training the GAN model
        :param data_loader_medrec_test: data loader for training MedRec
        :param save_model_path:
        :param n_epoch:
        :param print_every_iteration:
        :param save_every_epoch:
        :param gan_single_iteration: #iterations the GAN model has been trained after the MedRec has been trained for a single iteration
        :param proportion_bce:
        :param proportion_multi:
        :param trained_epoch: #epochs the model has already been trained
        :param trained_medrec_iterations: #iterations the model has already been trained
        :param trained_gan_iterations: #iterations the GAN model has already been trained
        :param gan_patient_ddi_rate: patients with DDI rate smaller than gan_patient_ddi_rate are used to fit the Gaussian distribution
        """
    ...
    def train(self, input_size, hidden_size, encoder_n_layers, encoder_embedding_dropout_rate, encoder_gru_dropout_rate,
        ...
        :param hidden_size:
        :param encoder_n_layers:
        :param encoder_embedding_dropout_rate:
        :param encoder_gru_dropout_rate:
        :param encoder_bidirectional:
        :param encoder_regular_lambda:
        :param encoder_learning_rate:
        :param decoder_dropout_rate:
        :param decoder_regular_lambda:
        :param decoder_learning_rate:
        :param hop:
        :param attn_type_kv:
        :param attn_type_embedding:
        :param gan_lr: learning rate for the generator optimizer and discriminator optimizer
        :param gan_regular_lambda: weight decay for the generator optimizer and discriminator optimizer
        :param gan_single_iteration:
        :param discriminator_dropout_rate: dropout rate for the discriminator
        :param discriminator_n_hidden_layers: #layers in the discriminator
        :param discriminator_dim_B: parameter for mini-batch discrimination in the discriminator
        :param discriminator_dim_C: parameter for mini-batch discrimination in the discriminator
        :param medrec_patient_ddi_rate: patient records whose DDi rate is smaller than medrec_patient_ddi_rate are used to train MedRec model
        :param gan_patient_ddi_rate: patient records whose DDi rate is smaller than medrec_patient_ddi_rate are used to generate real data for GAN model
        :param proprotion_bce:
        :param proportion_multi:
        :param real_data_file:
        :param save_model_path:
        :param n_epoch:
        :param print_every_iteration:
        :param save_every_epoch:
        ...
        # set save_path
        save_model_structure = str(encoder_n_layers) + '_' + str(input_size) + '_' + str(hidden_size) + '_' + str(
            encoder_bidirectional) + '_' + str(attn_type_kv) + '_' + str(attn_type_embedding) + '_MLPPremium_' + str(
            discriminator_n_hidden_layers)
        ...
        self.train_iters(encoder, decoder, discriminator, encoder_optimizer, decoder_optimizer, generator_optimizer,
                         discriminator_optimizer, data_loader_medrecGAN,
...
def MedRecTraining(input_size, hidden_size, encoder_n_layers, encoder_embedding_dropout_rate, encoder_gru_dropout_rate,
                   encoder_bidirectional, encoder_regular_lambda, encoder_learning_rate, decoder_dropout_rate,
                   decoder_regular_lambda, decoder_learning_rate, hop, attn_type_kv, attn_type_embedding,
                   save_model_dir, proportion_bce, proportion_multi, medrec_patient_ddi_rate, n_epoch=40,
                   print_every_iteration=100, save_every_epoch=5, load_model_name=None,
                   pretrained_embedding_diagnoses=None, pretrained_embedding_procedures=None,
                   pretrained_embedding_medications=None, discriminator_structure=None, discriminator_parameters=None):
    ...
    module.train(input_size, hidden_size, encoder_n_layers, encoder_embedding_dropout_rate, encoder_gru_dropout_rate,
                 ...
                 proportion_bce, proportion_multi, medrec_patient_ddi_rate,
...
def MedRecGANTraining(batch_size, input_size, hidden_size, encoder_n_layers,
                      encoder_embedding_dropout_rate, encoder_gru_dropout_rate, encoder_bidirectional,
                      encoder_regular_lambda, encoder_learning_rate, decoder_dropout_rate, decoder_regular_lambda,
                      decoder_learning_rate, hop, attn_type_kv, attn_type_embedding, gan_lr, gan_regular_lambda,
                      gan_single_iteration, discriminator_dropout_rate, discriminator_n_hidden_layers,
                      discriminator_dim_B, discriminator_dim_C, medrec_patient_ddi_rate, gan_patient_ddi_rate,
                      proprotion_bce, proportion_multi, real_data_file, save_model_path, n_epoch=40,
                      print_every_iteration=100, save_every_epoch=5, load_model_name=None,
                      pretrained_embedding_diagnoses=None, pretrained_embedding_procedures=None,
                      pretrained_embedding_medications=None):
    ...
    model.train(input_size, hidden_size, encoder_n_layers, encoder_embedding_dropout_rate, encoder_gru_dropout_rate,
                encoder_bidirectional, encoder_regular_lambda, encoder_learning_rate,
...
def fit_distribution_from_encoder(input_size, hidden_size, encoder_n_layers,
                                  encoder_bidirectional, load_model_name, medrec_patient_ddi_rate, real_data_save_path,
                                  encoder_structure_str, encoder_parameters_str, decimals=6, boxcox_transform=False,
    ...
    module.fit_gaussian_from_encoder(input_size, hidden_size, encoder_n_layers, encoder_bidirectional, load_model_name,
                                     medrec_patient_ddi_rate, real_data_save_path, encoder_structure_str,
                                     encoder_parameters_str, decimals, boxcox_transform,
...
def get_hidden_states_between_threshold(input_size, hidden_size, encoder_n_layer, encoder_bidirectional,
                                        ...
                                        save_data_path, encoder_structure_str, encoder_parameters_str):
    module = TrainMedRec(params.device, params.PATIENT_RECORDS_FILE_ACCUMULATE, params.DDI_MATRIX_FILE,
                         params.CONCEPTID_FILE, params.EHR_MATRIX_FILE)
    module.get_hidden_states_between_thresholds(input_size, hidden_size, encoder_n_layer, encoder_bidirectional,
                                                ...
                                                ddi_rate_high, save_data_path, encoder_structure_str,
                                                encoder_parameters_str)


if __name__ == '__main__':
    # MedRecGANTraining(8, 64, 64, 2, 0.1, 0.1, False, 0, 0.0001, 0.1, 0, 0.0001, 7, 'general', 'general', 0.0001, 0, 2,
    #                   0.1, 2, 10, 10, 0.3, 0.5, 0.9, 0.1, params.fitted_distribution_file_name, 'data/test/medrecgan',
    #                   4, 100, 2)
    patient_info=np.load('data/')
