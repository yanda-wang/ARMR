...
class MedRecSeq2Set(nn.Module):
    """
    warp the encoder and decoder into a single model
    """

    def __init__(self, device, input_size, hidden_size, **kwargs):
        super().__init__()

        self.encoder = EncoderRNNQuery(device=device, input_size=input_size, hidden_size=hidden_size,
                                       **params_for('encoder', kwargs))
        self.decoder = DecoderMultiHopKeyValueSeparate(device=device, hidden_size=hidden_size,
...
class MedRecTrainer(skorch.NeuralNet):
    """
    warp the encoder-decoder model into skorch for hyper-parameter tuning
    """
